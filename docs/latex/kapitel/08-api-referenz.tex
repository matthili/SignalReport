...
Wenn die Authentifizierung aktiviert ist, muessen alle API-Aufrufe einen
...
\hline
GET & \texttt{/api/statistics} & Statistiken fuer einen Messtyp.
  Parameter: \texttt{type} (PING|DNS|HTTP), \texttt{hours} (Zeitraum) \\
\hline
GET & \texttt{/api/hourly} & Stuendliche Durchschnittswerte fuer Heatmap.
...
\textbf{Methode} & \textbf{Pfad} & \textbf{Beschreibung} \\
\hline
GET & \texttt{/api/ip-changes} & Letzte IP-Aenderungen.
  Parameter: \texttt{limit} \\
\hline
GET & \texttt{/api/ip-change-stats} & Statistik ueber IP-Aenderungen
  pro Host \\
\hline
...
\hline
POST & \texttt{/api/theme/settings} & Theme aendern.
...
\textbf{Methode} & \textbf{Pfad} & \textbf{Beschreibung} \\
\hline
POST & \texttt{/api/setup/complete} & Ersteinrichtung abschliessen.
...
\hline
POST & \texttt{/api/auth/change-password} & Admin-Passwort aendern \\
\hline
\end{tabular}
...
\hline
POST & \texttt{/api/push/settings} & Push-Einstellungen aendern
  (Schwellwert, Anzahl) \\
\hline
POST & \texttt{/api/push/subscribe} & Client fuer Push registrieren \\
\hline
