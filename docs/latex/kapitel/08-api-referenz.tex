...
GET & \texttt{/api/measurements} & Letzte Messungen abrufen.
  Parameter: \texttt{limit} (Anzahl, Standard: 10) \\
\hline
GET & \texttt{/api/statistics} & Statistiken für einen Messtyp.
  Parameter: \texttt{type} (PING|DNS|HTTP), \texttt{hours} (Zeitraum) \\
\hline
GET & \texttt{/api/hourly-averages} & Stündliche Durchschnittswerte für Heatmap.
...
\subsection{Verfügbarkeit und Störungs-Lokalisierung}

Diese Endpunkte werden serverseitig von \texttt{ReliabilityRoutes} (Paket \texttt{web.api}) registriert; Pfade und Verhalten sind gegenüber der monolithischen Variante unverändert.
...
\textbf{Methode} & \textbf{Pfad} & \textbf{Beschreibung} \\
\hline
GET & \texttt{/api/connectivity} & Störungs-Lokalisierung: vergleicht nahen/fernen Gateway mit dem Internet-Ping.
  Parameter: \texttt{hours}. Liefert \texttt{verdict} (Router/Gateway/Internet) und \texttt{segments} \\
\hline
GET & \texttt{/api/reliability} & Lückenbewusste Kennzahlen.
  Parameter: \texttt{hours}. Liefert Verfügbarkeit, Abdeckung, MTBF, MTTR und die aggregierte Ausfallliste \\
\hline
POST & \texttt{/api/outages/exclude} & Ausfallfenster aus der Wertung nehmen bzw.\ wieder aufnehmen.
  Body: \texttt{\{fromEpoch, toEpoch, excluded\}} (setzt die DB-Spalte \texttt{excluded}) \\
\hline
\end{tabular}
\caption{API-Endpunkte: Verfügbarkeit und Störungs-Lokalisierung}
\end{table}

\subsection{Dienst-Erreichbarkeit}

Diese Endpunkte werden von \texttt{ServiceReachabilityRoutes} (Paket \texttt{web.api}) registriert. Das Feature ist standardmäßig deaktiviert; ist es aus, liefert \texttt{/api/services} eine leere Dienstliste mit \texttt{enabled=false}.
...
\textbf{Methode} & \textbf{Pfad} & \textbf{Beschreibung} \\
\hline
GET & \texttt{/api/services} & Aktueller Status je aktivem Dienst (Verdikt, \enquote{seit wann}, Erreichbarkeitsquote, letzte Methode) für Ampel/Kacheln.
  Parameter: \texttt{days} (Zeitfenster, Standard 30) \\
\hline
GET & \texttt{/api/services/history} & Episoden (Zustandswechsel) eines Dienstes -- die Sperr-Timeline.
  Parameter: \texttt{id} (Dienst), \texttt{days} (Standard 90) \\
\hline
GET & \texttt{/api/services/settings} & Einstellungen abrufen: Hauptschalter, Intervall, Kontroll-SNI, vollständige Dienstliste \\
\hline
POST & \texttt{/api/services/settings} & Einstellungen speichern.
  Body: \texttt{\{enabled, intervalMinutes, useControlSni, services:[\{id, enabled\}]\}} \\
\hline
POST & \texttt{/api/services/check-now} & Sofortige Prüfung auslösen.
  Liefert \texttt{cooldownRemainingSeconds} (0 = gestartet; 5-Minuten-Abkühlphase) \\
\hline
\end{tabular}
\caption{API-Endpunkte: Dienst-Erreichbarkeit}
\end{table}
...
Der DNS-Benchmark vergleicht die Antwortzeiten aller konfigurierten DNS-Server
(über 30 weltweit) durch parallele Abfragen mittels Virtual Threads. Ein
30-Sekunden-Cooldown verhindert übermäßige Belastung.
...
SignalReport protokolliert automatisch Änderungen der lokalen und externen
IP-Adressen. Über das Host-Hash-System können mehrere Installationen
unterschieden werden, ohne personenbezogene Daten zu speichern.
...
Alle Einstellungen (Messziele, Intervall, Wartungsfenster, Theme) werden
über die Konfigurations-Endpunkte verwaltet und in \texttt{config.json}
persistiert. Änderungen werden sofort wirksam, ohne Neustart.
...
GET & \texttt{/api/config/current} & Aktuelle Konfiguration abrufen
  (Messziele, Intervall, Maintenance, Benutzer-Info, Gateway-Objekt inkl.\ \texttt{virtualSuspected}, Sprache) \\
\hline
POST & \texttt{/api/config/update} & Konfiguration aktualisieren.
  Body: JSON mit ping, dns, http, intervalSeconds, maintenance, userInfo sowie \texttt{gateway}
  (nearManual, near, nearPersistent, farManual, far, farPersistent, farPingEnabled) \\
\hline
POST & \texttt{/api/config/language} & UI-Sprache umstellen. Body: \texttt{\{language\}} (ISO-Code) \\
\hline
...
Diese Endpunkte steuern die Ersteinrichtung (Setup-Wizard) sowie das Aktivieren,
Deaktivieren und Ändern der Authentifizierung. Alle sicherheitsrelevanten Aktionen
erfordern eine Challenge-Response-Verifikation -- auch das Deaktivieren der
Authentifizierung.
...
\hline
GET & \texttt{/api/setup/status} & Status der Ersteinrichtung abfragen (abgeschlossen: ja/nein) \\
\hline
...
Browser-Push-Benachrichtigungen informieren den Benutzer automatisch, wenn die
Verbindungsqualität unter einen konfigurierbaren Schwellwert fällt oder wiederholt
Messungen fehlschlagen.
