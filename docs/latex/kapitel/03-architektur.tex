...
\begin{description}
  ...
  \item[network] \texttt{GatewayDiscovery} (Traceroute-Gateway-Kette nah/fern,
    ...
    \texttt{HostIdentifier} (stabiler Host-Hash).
  \item[storage] \texttt{H2MeasurementRepository} für die embedded Datenbank in
    ...
    Tabellen: Messungen, Hosts, IP-Änderungen. Siehe Abschnitt \ref{subsec:twindb}.
  \item[report] \texttt{ReliabilityReport} (lückenbewusste Kennzahlen),
    \texttt{ConnectivityAssessment} (Schuld-Verdikt) und \texttt{PdfReportGenerator}
    (OpenPDF + JFreeChart).
  ...
  \item[web.api] Neun Routen-Registrar-Klassen (\texttt{PageRoutes}, \texttt{MeasurementRoutes},
...
\begin{itemize}
  ...
  \item Die \textbf{Renderer- und Routen-Aufteilung}: Das HTML-Rendering wurde in
    ...
    nach Domäne in neun Registrar-Klassen (Paket \texttt{web.api}) zerlegt. Der
