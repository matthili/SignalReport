...
SignalReport ist als einzelne ausfuehrbare JAR-Datei konzipiert und benoetigt lediglich
...
\begin{itemize}
  ...
  \item \textbf{Speicherplatz:} ca.\ 50\,MB fuer die Anwendung; Datenbank waechst mit der
...
Anschliessend ist das Web-Interface unter \texttt{http://localhost:4567} erreichbar.
Beim ersten Start wird automatisch der Setup-Assistent angezeigt.

\textbf{Hinweis:} Bei dieser Variante laeuft SignalReport nur, solange das Terminal-Fenster
geoeffnet bleibt. Fuer den Dauerbetrieb empfiehlt sich die Installation als Dienst.

\subsection{Installation als Windows-Dienst}
\label{subsec:install-windows}

Das Projekt enthaelt ein Installations-Skript unter
...
\begin{enumerate}
  \item Skript als Administrator ausfuehren (Rechtsklick $\rightarrow$ ``Als Administrator
    ausfuehren'')
  \item Automatische Pruefung: Administrator-Rechte und Java-Installation
  ...
  \item Erstellung einer Desktop-Verknuepfung
  \item Einrichtung einer Firewall-Regel fuer Port 4567
\end{enumerate}
...
Die Deinstallation erfolgt ueber \texttt{deployment/windows/uninstall.bat} (ebenfalls als
Administrator).
...
\begin{enumerate}
  \item Ausfuehrung mit \texttt{sudo ./install.sh}
  ...
  \item Verzeichnisstruktur:
    ...
    \end{itemize}
  \item Optional: Kompilierung von jsvc (Apache Commons Daemon) fuer professionellen
...
\begin{itemize}
  ...
  \item Desktop-Verknuepfung: \texttt{.webloc}-Datei auf dem Schreibtisch
  \item Laeuft als angemeldeter Benutzer (nicht als separater Systembenutzer)
\end{itemize}
...
erstellt. Alle Einstellungen koennen ueber das Web-Interface geaendert werden; alternativ
...
Fuer ein vollstaendiges Backup genuegt es, zwei Dateien/Verzeichnisse zu sichern:
